...
class Quiz extends CI_Controller {
    ...
    public function start($encoded_quiz_id) {
        $quiz_id = decode_id($encoded_quiz_id);
        if (!$quiz_id) show_404();
        ...
        $attempt = array('quiz_id' => $quiz_id, 'user_id' => $user_id, 'started_at' => date('Y-m-d H:i:s'));
        $attempt_id = $this->Quiz_model->create_attempt($attempt);
        redirect('quiz/take/' . encode_id($attempt_id));
    }

    public function take($encoded_attempt_id) {
        $attempt_id = decode_id($encoded_attempt_id);
        ...
        if ($attempt->finished_at) redirect('quiz/result/' . $encoded_attempt_id);
        $data['title'] = t('Quiz: ', 'Quiz: ') . $quiz->title;
        $data['quiz'] = $quiz;
        $data['questions'] = $questions;
        $data['attempt'] = $attempt;
        $data['encoded_attempt_id'] = $encoded_attempt_id;
        $this->load->view('templates/header', $data);
        $this->load->view('quiz/take', $data);
        $this->load->view('templates/footer');
    }

    public function submit($encoded_attempt_id) {
        ...
        if ($quiz->time_limit > 0 && $attempt->started_at) {
            ...
            if (time() > $max_end) $this->session->set_flashdata('error', t('Waktu pengerjaan sudah habis.', 'Time is up.'));
        }
        $score = 0; $total_points = 0; $answers = array();
        foreach ($questions as $q) {
            ...
            if (in_array($q->question_type, ['multiple_choice', 'true_false'])) {
                ...
                if (strtolower(trim($answer)) === strtolower(trim($q->correct_answer))) $score += $q->points;
            }
        }
        ...
        $this->Quiz_model->update_attempt($attempt_id, array(
            ...
            'answers' => json_encode($answers), 'finished_at' => date('Y-m-d H:i:s'), 'is_passed' => $passed ? 1 : 0
        ));
        if ($quiz->lesson_id) $this->Course_model->mark_lesson_completed($this->session->userdata('user_id'), $quiz->lesson_id);
        if ($passed) award_points($this->session->userdata('user_id'), 25, 'quiz_passed', $quiz->id);
        redirect('quiz/result/' . $encoded_attempt_id);
    }

    public function result($encoded_attempt_id) {
        $attempt_id = decode_id($encoded_attempt_id);
        if (!$attempt_id) show_404();
        $attempt = $this->Quiz_model->get_attempt_by_id($attempt_id);
        if (!$attempt || $attempt->user_id != $this->session->userdata('user_id')) show_404();
    ...
    public function admin_quizzes($course_id) {
        ...
        $qcounts = array();
        foreach ($data['quizzes'] as $qz) $qcounts[$qz->id] = $this->Quiz_model->count_questions($qz->id);
    ...
    public function admin_create_quiz($course_id) {
        ...
        if ($this->form_validation->run() === FALSE) {
            ...
            $this->Quiz_model->create_quiz(array(
                ...
                'title' => $this->input->post('title'), 'passing_score' => $this->input->post('passing_score'),
                'time_limit' => $this->input->post('time_limit') ?: 0, 'max_attempts' => $this->input->post('max_attempts') ?: 3
            ));
    ...
    public function admin_delete_quiz($id) {
        ...
        redirect('quiz/admin_quizzes/' . $quiz->course_id);
    }
    ...
    public function admin_create_question($quiz_id) {
        ...
        if ($this->form_validation->run() === FALSE) {
            ...
            if ($this->input->post('type') === 'multiple_choice') {
                ...
                $opts = array();
                if ($opt_text) { foreach (explode("\n", $opt_text) as $line) { $line = trim($line); if ($line) $opts[] = $line; } }
                $options = json_encode($opts);
            }
            $this->Quiz_model->create_question(array(
                'quiz_id' => $quiz_id, 'question' => $this->input->post('question'),
                'question_en' => $this->input->post('question_en') ?: '',
                'type' => $this->input->post('type'), 'options' => $options,
                'correct_answer' => $this->input->post('correct_answer'), 'points' => $this->input->post('points') ?: 1,
    ...
    public function admin_delete_question($question_id) {
        ...
        redirect('quiz/admin_questions/' . $q->quiz_id);
    }
}
