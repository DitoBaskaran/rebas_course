...
class Assignment extends CI_Controller {
    ...
    public function submit($assignment_id) {
        ...
        if ($assignment->due_days > 0) {
            ...
            if ($enrolled) {
                ...
                if (date('Y-m-d') > $due_date) {
                    ...
                    redirect('assignment/view/' . $assignment_id);
                }
            }
        }
        ...
        if (!empty($_FILES['submission_file']['name'])) {
            ...
            // Use per-assignment file type and size settings
            ...
            $config = array(
                ...
                'max_size' => $max_size,
                'file_name' => 'submission_' . $assignment_id . '_' . $user_id . '_' . time()
            );
        ...
        if ($existing) {
            $this->Assignment_model->update_submission($existing->id, $data);
        } else {
            $this->Assignment_model->create_submission($data);
        }

        // Auto-mark lesson complete if linked
