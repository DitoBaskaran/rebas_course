...
class Mentor_dashboard extends CI_Controller {
    ...
    public function add_slot() {
        ...
        if ($this->form_validation->run()) {
            $this->Mentor_availability_model->create(array(
                'mentor_id' => $mentor->id,
                'day_of_week' => $date_override ? null : $day_of_week,
    ...
    public function confirm_session($session_id) {
        $mentor = $this->get_mentor_profile();
        ...
        if ($session && $session->mentor_id == $mentor->id) {
            $this->Mentoring_bookings_model->update($session_id, array(
                'status' => 'confirmed',
                'mentor_confirmed_at' => date('Y-m-d H:i:s'),
            ));
    ...
    public function complete_session($session_id) {
        ...
        if ($session && $session->mentor_id == $mentor->id && in_array($session->status, array('confirmed'))) {
            ...
            $this->session->set_flashdata('success', t('Sesi ditandai selesai.', 'Session marked as completed.'));
        }
        redirect('mentor/sessions');
    }

    public function rate_user($session_id) {
        $mentor = $this->get_mentor_profile();
        $session = $this->Mentoring_bookings_model->get_by_id($session_id);
