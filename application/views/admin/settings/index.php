<div class="container-fluid py-4" style="max-width: 1400px;">
    ...
    <div class="border rounded-3 p-3" style="border-color: #e7e5e4; border-radius: 12px;">
        <div class="d-flex align-items-center gap-2 px-1 py-2 mb-3" style="border-bottom: 1px solid #f0eeeb;">
            <i class="fas fa-sliders-h" style="color: #f97316; font-size: 0.8rem;"></i>
            <span class="fw-semibold" style="color: #1c1917; font-size: 0.9rem;"><?php echo $page_title; ?></span>
        </div>
        ...
        <div class="d-flex flex-column gap-4 p-3">
            ...
            foreach ($settings as $s):
                if ($active_group === 'payment' && isset($payment_methods_config[$s->key])):
                    ...
                    $payment_cards .= '
                    <div class="d-flex align-items-center gap-3 p-3 rounded-3" style="background: #fafaf9; border: 1px solid #f0eeeb;">
                        ' . $logo_html . '
                        <div class="flex-fill">
                            <div class="fw-semibold" style="color: #1c1917; font-size: 0.82rem;">' . $pm['label'] . '</div>
            ...
            <?php foreach ($other_settings as $s): ?>
                <div>
                    <?php if ($s->type === 'boolean'): ?>
                        <label class="fw-semibold mb-1 d-block" style="color: #1c1917; font-size: 0.82rem;"><?php echo htmlspecialchars($s->label ?: $s->key); ?></label>
                    ...
                    <?php elseif ($s->type === 'image'): ?>
                        <label class="fw-semibold mb-1 d-block" style="color: #1c1917; font-size: 0.82rem;"><?php echo htmlspecialchars($s->label ?: $s->key); ?></label>
                        <div class="d-flex align-items-center gap-3">
                            <div style="width: 80px; height: 80px; border-radius: 8px; border: 2px dashed #e7e5e4; display: flex; align-items: center; justify-content: center; overflow: hidden; background: #fafaf9;">
        ...
        <div class="d-flex justify-content-end gap-2 pt-3" style="border-top: 1px solid #f0eeeb;">
            ...
            <button type="submit" class="btn px-4 py-2 fw-bold rounded-pill d-flex align-items-center gap-1" style="background: #f97316; color: #fff; font-size: 0.82rem;">
