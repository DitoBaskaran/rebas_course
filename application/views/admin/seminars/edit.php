<div class="container-fluid px-0">
    ...
    <div class="bento-card mb-4 overflow-hidden" style="background:linear-gradient(120deg,#0D1830 0%,#164e63 100%);border:none;color:#fff;">
        <div class="position-relative" style="z-index:1;">
            ...
            <span class="d-inline-flex align-items-center gap-1" style="background:rgba(255,255,255,0.14);border:1px solid rgba(255,255,255,0.2);color:#FBBF24;font-size:0.66rem;font-weight:800;letter-spacing:0.12em;text-transform:uppercase;padding:0.3rem 0.7rem;border-radius:100px;">
                ...
                <?php echo t('Edit Seminar', 'Edit Seminar'); ?>
            </span>
            ...
            <div class="d-flex flex-wrap gap-2 mt-1">
                <?php $upcoming = strtotime($seminar->date_time) > time(); ?>
                <span class="d-inline-flex align-items-center gap-1 px-2 py-1 rounded-pill fw-semibold" style="background:<?php echo $upcoming ? '#E0F2F1' : '#f1f5f9'; ?>;color:<?php echo $upcoming ? '#009688' : '#64748b'; ?>;font-size:0.68rem;"><?php echo $upcoming ? t('Mendatang', 'Upcoming') : t('Selesai', 'Ended'); ?></span>
                <span class="d-inline-flex align-items-center gap-1 px-2 py-1 rounded-pill fw-semibold" style="background:rgba(255,255,255,0.14);color:#fff;font-size:0.68rem;"><?php echo date('d M Y, H:i', strtotime($seminar->date_time)); ?></span>
                <span class="d-inline-flex align-items-center gap-1 px-2 py-1 rounded-pill fw-semibold" style="background:rgba(255,255,255,0.14);color:#fff;font-size:0.68rem;">Rp <?php echo number_format($seminar->price, 0, ',', '.'); ?></span>
            </div>
        </div>
    </div>
    ...
    <?php echo form_open_multipart('admin/edit_seminar/' . $seminar->id, array('class' => 'needs-validation', 'id' => 'seminarForm')); ?>
            ...
            <div class="bento-card">
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="title" id="pv_title" class="form-control" value="<?php echo set_value('title', $seminar->title); ?>" required style="border-radius:12px;font-size:0.88rem;height:44px;">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label fw-semibold small"><?php echo t('Judul (English)', 'Title (English)'); ?></label>
                        <input type="text" name="title_en" class="form-control" value="<?php echo set_value('title_en', $seminar->title_en ?? ''); ?>" style="border-radius:12px;font-size:0.88rem;height:44px;">
                    </div>
                </div>
            </div>
            ...
            <div class="bento-card">
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="datetime-local" name="date_time" id="pv_date" class="form-control" value="<?php echo set_value('date_time', date('Y-m-d\TH:i', strtotime($seminar->date_time))); ?>" required style="border-radius:12px;font-size:0.85rem;height:44px;">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label fw-semibold small"><?php echo t('Kuota Peserta', 'Participant Quota'); ?> *</label>
                        <input type="number" name="quota" id="pv_quota" class="form-control" value="<?php echo set_value('quota', round($seminar->quota)); ?>" required min="1" style="border-radius:12px;font-size:0.85rem;height:44px;">
                    </div>
                </div>
            </div>
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <div class="input-group">
                            ...
                            <input type="number" name="price" id="pv_price" class="form-control" value="<?php echo set_value('price', round($seminar->price)); ?>" required min="0" step="1000" style="border-radius:0 12px 12px 0;font-size:0.88rem;height:44px;">
                        </div>
            ...
            <div class="bento-card">
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="url" name="location_link" class="form-control" value="<?php echo set_value('location_link', $seminar->location_link ?? ''); ?>" placeholder="https://zoom.us/..." style="border-radius:12px;font-size:0.85rem;height:44px;">
                        <small class="field-hint"><?php echo t('Opsional — ditampilkan setelah pendaftaran.', 'Optional — shown after registration.'); ?></small>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label fw-semibold small"><?php echo t('Poster Baru', 'New Poster'); ?></label>
            ...
            <div class="bento-card">
                ...
                <div class="d-flex flex-column gap-3">
                    <div>
                        ...
                        <textarea name="description" rows="5" class="form-control" required data-max-chars="2000" style="border-radius:12px;font-size:0.85rem;line-height:1.65;"><?php echo set_value('description', $seminar->description); ?></textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="description_en" rows="4" class="form-control" data-max-chars="2000" style="border-radius:12px;font-size:0.85rem;line-height:1.65;"><?php echo set_value('description_en', $seminar->description_en ?? ''); ?></textarea>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="bento-card">
                ...
                <div class="seminar-card" style="border:1px solid var(--card-border,#e7e5e4);border-radius:16px;overflow:hidden;">
                    <div class="position-relative" style="aspect-ratio:16/9;background:linear-gradient(135deg,#0D1830,#164e63);overflow:hidden;">
                        <img id="pv_thumb_img" src="<?php echo base_url('uploads/seminars/' . $seminar->thumbnail); ?>" alt="" style="width:100%;height:100%;object-fit:cover;" onerror="this.style.display='none';document.getElementById('pv_thumb_placeholder').style.display='flex';">
                    ...
                    <div class="px-3 py-3">
                        <div class="fw-bold text-dark" style="font-size:0.9rem;line-height:1.35;" id="pv_title_out"><?php echo htmlspecialchars($seminar->title); ?></div>
                        <div class="d-flex align-items-center justify-content-between mt-2">
                            <span class="text-secondary" style="font-size:0.72rem;" id="pv_date_out"><?php echo date('d M Y', strtotime($seminar->date_time)); ?> · <?php echo date('H:i', strtotime($seminar->date_time)); ?></span>
                            <span class="fw-extrabold" style="color:<?php echo $seminar->price > 0 ? '#0D1830' : '#009688'; ?>;font-size:0.92rem;" id="pv_price_out"><?php echo $seminar->price > 0 ? 'Rp ' . number_format($seminar->price, 0, ',', '.') : t('Gratis', 'Free'); ?></span>
                        </div>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function upd() {
        var t = el('pv_title');
        el('pv_title_out').textContent = (t && t.value.trim()) ? t.value.trim() : '<?php echo htmlspecialchars($seminar->title, ENT_QUOTES); ?>';
        var d = el('pv_date');
