<div class="container-fluid py-4" style="max-width: 1400px;">
    <div class="d-flex flex-column flex-md-row align-items-md-center justify-content-between mb-4 gap-3">
        <div>
            <div style="color: #0D1830; font-weight: 700; font-size: 0.7rem; text-transform: uppercase; letter-spacing: 0.08em; margin-bottom: 0.15rem;"><?php echo t('Keuangan', 'Finance'); ?></div>
            <h4 class="fw-extrabold mb-0" style="color: #0D1830; letter-spacing: -0.02em; font-size: 1.4rem;"><?php echo t('Transaksi', 'Transactions'); ?></h4>
    ...
    <div class="border rounded-3" style="border-color: #e7e5e4; border-radius: 12px; overflow: hidden;">
        <?php if (empty($transactions)): ?>
            <div class="p-5 text-center"><div style="font-size: 2rem; color: #d6d3d1; margin-bottom: 0.5rem;"><i class="fas fa-receipt"></i></div><h6 class="fw-bold" style="color: #0D1830;"><?php echo t('Belum ada transaksi.', 'No transactions yet.'); ?></h6></div>
        <?php else: ?>
                ...
                <table class="table mb-0" style="font-size: 0.8rem;">
                    <thead>
                        <tr>
                            ...
                            <th style="font-weight: 600; color: #78716c; font-size: 0.68rem; border-color: #e7e5e4; padding: 0.7rem 1rem; background: #E6EBEF; text-transform: uppercase; letter-spacing: 0.05em;"><?php echo t('Pengguna', 'User'); ?></th>
                            <th style="font-weight: 600; color: #78716c; font-size: 0.68rem; border-color: #e7e5e4; padding: 0.7rem 1rem; background: #E6EBEF; text-transform: uppercase; letter-spacing: 0.05em;"><?php echo t('Item', 'Item'); ?></th>
                            ...
                            <th style="font-weight: 600; color: #78716c; font-size: 0.68rem; border-color: #e7e5e4; padding: 0.7rem 1rem; background: #E6EBEF; text-transform: uppercase; letter-spacing: 0.05em;"><?php echo t('Jumlah', 'Amount'); ?></th>
                            <th style="font-weight: 600; color: #78716c; font-size: 0.68rem; border-color: #e7e5e4; padding: 0.7rem 1rem; background: #E6EBEF; text-transform: uppercase; letter-spacing: 0.05em;"><?php echo t('Kupon', 'Coupon'); ?></th>
                            ...
                            <th style="font-weight: 600; color: #78716c; font-size: 0.68rem; border-color: #e7e5e4; padding: 0.7rem 1rem; background: #E6EBEF; text-transform: uppercase; letter-spacing: 0.05em;"><?php echo t('Tanggal', 'Date'); ?></th>
                            <th style="font-weight: 600; color: #78716c; font-size: 0.68rem; border-color: #e7e5e4; padding: 0.7rem 1rem; background: #E6EBEF; text-transform: uppercase; letter-spacing: 0.05em; text-align: center; width: 90px;"><?php echo t('Aksi', 'Action'); ?></th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php foreach ($transactions as $tx): ?>
                            <tr>
                                <td style="border-color: #f0eeeb; padding: 0.65rem 1rem; font-family: monospace; font-size: 0.72rem; color: #0D1830;">BT-<?php echo $tx->uuid; ?></td>
                                <td style="border-color: #f0eeeb; padding: 0.65rem 1rem; font-weight: 600; color: #0D1830; font-size: 0.78rem;"><?php echo htmlspecialchars($tx->user_name); ?></td>
                                ...
                                <td style="border-color: #f0eeeb; padding: 0.65rem 1rem;"><span class="px-2 py-1 rounded-pill fw-semibold" style="background: #E6EBEF; color: #57534e; font-size: 0.6rem;"><?php echo $item_labels[$tx->item_type] ?? ucfirst($tx->item_type); ?></span></td>
                                <td style="border-color: #f0eeeb; padding: 0.65rem 1rem; font-weight: 700; color: #0D1830; font-size: 0.8rem;">Rp <?php echo number_format($tx->amount, 0, ',', '.'); ?><?php if ($tx->discount_amount > 0): ?><br><small style="color: #009688; font-size: 0.65rem;">(-Rp <?php echo number_format($tx->discount_amount, 0, ',', '.'); ?>)</small><?php endif; ?></td>
                                <td style="border-color: #f0eeeb; padding: 0.65rem 1rem;"><?php if ($tx->coupon_id): $coupon = $this->db->get_where('coupons', array('id' => $tx->coupon_id))->row(); ?><span class="px-2 py-1 rounded-pill fw-semibold" style="background: #E0F2F1; color: #009688; font-size: 0.6rem;"><?php echo $coupon ? htmlspecialchars($coupon->code) : '#' . $tx->coupon_id; ?></span><?php else: ?><span style="color: #a8a29e; font-size: 0.72rem;">-</span><?php endif; ?></td>
                                <td style="border-color: #f0eeeb; padding: 0.65rem 1rem;"><?php if ($tx->status === 'pending'): ?><span class="px-2 py-1 rounded-pill fw-semibold" style="background: #fff7ed; color: #0D1830; font-size: 0.6rem;"><?php echo t('Pending', 'Pending'); ?></span><?php elseif ($tx->status === 'approved'): ?><span class="px-2 py-1 rounded-pill fw-semibold" style="background: #E0F2F1; color: #009688; font-size: 0.6rem;"><?php echo t('Disetujui', 'Approved'); ?></span><?php else: ?><span class="px-2 py-1 rounded-pill fw-semibold" style="background: #fef2f2; color: #f43f5e; font-size: 0.6rem;"><?php echo t('Ditolak', 'Rejected'); ?></span><?php endif; ?></td>
