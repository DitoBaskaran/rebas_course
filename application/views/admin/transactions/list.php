                    ...
                    <tbody>
                        <?php foreach ($transactions as $tx): ?>
                            <tr>
                                ...
                                <td style="border-color: #f0eeeb; padding: 0.65rem 1rem; font-weight: 700; color: #1c1917; font-size: 0.8rem;">Rp <?php echo number_format($tx->amount, 0, ',', '.'); ?><?php if ($tx->discount_amount > 0): ?><br><small style="color: #14b8a6; font-size: 0.65rem;">(-Rp <?php echo number_format($tx->discount_amount, 0, ',', '.'); ?>)</small><?php endif; ?></td>
                                <td style="border-color: #f0eeeb; padding: 0.65rem 1rem;"><?php if ($tx->coupon_id): $coupon = $this->db->get_where('coupons', array('id' => $tx->coupon_id))->row(); ?><span class="px-2 py-1 rounded-pill fw-semibold" style="background: #f0fdfa; color: #14b8a6; font-size: 0.6rem;"><?php echo $coupon ? htmlspecialchars($coupon->code) : '#' . $tx->coupon_id; ?></span><?php else: ?><span style="color: #a8a29e; font-size: 0.72rem;">-</span><?php endif; ?></td>
                                <td style="border-color: #f0eeeb; padding: 0.65rem 1rem;"><?php if ($tx->status === 'pending'): ?><span class="px-2 py-1 rounded-pill fw-semibold" style="background: #fff7ed; color: #f97316; font-size: 0.6rem;"><?php echo t('Pending', 'Pending'); ?></span><?php elseif ($tx->status === 'approved'): ?><span class="px-2 py-1 rounded-pill fw-semibold" style="background: #f0fdfa; color: #14b8a6; font-size: 0.6rem;"><?php echo t('Disetujui', 'Approved'); ?></span><?php else: ?><span class="px-2 py-1 rounded-pill fw-semibold" style="background: #fef2f2; color: #f43f5e; font-size: 0.6rem;"><?php echo t('Ditolak', 'Rejected'); ?></span><?php endif; ?></td>
